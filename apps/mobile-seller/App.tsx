...
import { sellerTokenPersistence } from './src/lib/persistence';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    configureTokenPersistence(sellerTokenPersistence);
    hydrateAuthFromPersistence()
      .then((u) => {
        if (u && u.role !== 'SELLER') {
          logout().catch(() => {});
  ...
  const onLogin = async () => {
    ...
    try {
      ...
      if (res.user.role !== 'SELLER') {
        await logout();
        setError('This app is for seller accounts only. Use the customer or business app for other accounts.');
        return;
      }
      setUser(res.user);
    } catch (e: unknown) {
  ...
  if (user) {
    return (
      <View style={styles.container}>
        <Text style={styles.title}>Xelnova Seller</Text>
  ...
  return (
    ...
    >
      <Text style={styles.title}>Xelnova Seller</Text>
      <Text style={styles.sub}>Sign in with your seller account</Text>
      <TextInput
        style={styles.input}
        placeholder="Email"
        autoCapitalize="none"
