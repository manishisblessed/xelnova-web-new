...
export function ChartCard({ title, data, dataKey = 'value', loading }: ChartCardProps) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl border border-border bg-surface p-6 shadow-card">
      ...
      <div className="h-[240px]">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
            ...
            <Area type="monotone" dataKey={dataKey} stroke="var(--color-primary-500)" strokeWidth={2} fill="url(#sellerChartGradient)" />
          </AreaChart>
        </ResponsiveContainer>
      </div>
    </motion.div>
