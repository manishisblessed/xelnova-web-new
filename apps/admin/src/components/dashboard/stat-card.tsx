...
export interface StatCardProps {
   ...
   * Next.js `<Link>` so admins can deep-link straight from a KPI tile to
   * the relevant filtered list (e.g. "Pending Review" -> the onboarding
   * queue filtered by status). Wrapping the card itself rather than
   * decorating call sites with `<Link>` keeps every stat tile across the
   * app consistently navigable without each page having to re-implement
   * the focus/hover affordances.
   */
  href?: string;
  /**
        ...
        <div>
          ...
          {(change !== undefined || changeLabel) && (
            <p className="mt-1 text-xs text-text-muted">
              {change !== undefined && (
                <span className={change >= 0 ? 'text-success-600' : 'text-danger-500'}>
                  {change >= 0 ? '+' : ''}{change}%
                </span>
              )}
