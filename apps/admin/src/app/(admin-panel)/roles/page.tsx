  ...
  return (
    <>
      <AdminListPage<Role> title="Roles" section="roles" columns={columns} keyExtractor={(r) => r.id}
        searchKeys={['name']} onAdd={openAdd} addLabel="Create Role"
        renderActions={(r) => (
      ...
      <ActionModal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Role' : 'Create Role'}
        onSubmit={() => { toast.success(editing ? 'Role updated' : 'Role created'); setModalOpen(false); }}>
      ...
      <ConfirmDialog open={deleteOpen} onClose={() => setDeleteOpen(false)} onConfirm={() => { toast.success('Role deleted'); setDeleteOpen(false); }} title="Delete Role" message={`Delete role "${editing?.name}"? Users with this role will lose their permissions.`} />
    </>
  );
}
